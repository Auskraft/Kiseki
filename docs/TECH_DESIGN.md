...
> **Факт (отклонения от плана):** `bloc_concurrency` НЕ используется — пере-подписка на стрим сделана вручную (`MediaListCubit._resubscribe`). `go_router` ещё не добавлен (A4 отложен; навигация — `Navigator.push`). `drift_flutter` в зависимостях остался, но БД открывается через `NativeDatabase` напрямую (ADR-19) — можно убрать. Бэкап-пакеты добавлены и проверены на устройстве.
