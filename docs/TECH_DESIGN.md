...
- **Форма создания/редактирования** (модальный боттом-шит, ADR-22) — прогрессивное раскрытие: **Формат** (Одиночный/Серийный) → **Тип** (вид, подписи по формату) → **Название** → сворачиваемые **«Дополнительные параметры»** (статус, оригинал, год, страна, обложка, оценка, теги, прогресс `episodic`, причина при паузе/забросе, даты, заметка). Сохранение — иконкой-галочкой в шапке; крестика нет (закрытие свайпом). Поля серий/причины показываются реактивно от `format`/`status`.
