...
| `shared_preferences` | персист выбранной темы/режима (ThemeCubit) |
| dev: `build_runner`, `drift_dev` | кодоген Drift |
| dev: `bloc_test`, `mocktail` | тесты bloc на моках репозитория |

Отложено (вводим позже без лок-ина): `injectable`/`injectable_generator` (ADR-13), HTTP-клиент для Я.Диска (этап бэкапа). Переводы на другие языки — на бете (инфраструктура ключей уже стоит, ADR-15).
