...
| `media_type` | TEXT | `movie` · `series` · `drama` · `anime` |
...
- **ADR-07. `format` (`single`/`episodic`) — отдельное обязательное поле**, а не вывод из `media_type`. Причина: аниме бывает фильмом, веб-дорама — одиночкой. `format` гейтит блок серий и `CHECK`. Дефолт от типа (`movie→single`, прочие→`episodic`), пользователь переопределяет. *[Сведение: модель данных не имела `format` и вешала CHECK на `media_type='movie'` — исправлено.]*
...
- **ADR-20. Навигация — go_router, плоские маршруты, OS deep-linking выключен (A4).** `MaterialApp.router` + единый реестр путей `AppRoute` (`lib/app/router/app_router.dart`). Маршруты **плоские** (`/`, `/editor`, `/item/:id`, `/item/:id/edit`, `/settings`, `/settings/tags`, `/settings/trash`) — каждый путь = ровно одна страница, поэтому `context.push`/`context.pop` дают предсказуемый стек; карточка адресуется по UUID (`/item/<id>` — выполняет «deep-link на карточку» из DoD A4, передача аргументов через path-параметр). Роутер создаётся **один раз на монтирование дерева** (поле `State` в `KisekiApp`): переживает пересборку темы и сбрасывается на старт при Replace-all (RestartWidget перемонтирует дерево → новый роутер → после восстановления приложение открывается на главной). Прикладные блоки (`ThemeCubit`, `MediaListCubit`) живут выше `MaterialApp.router` → доступны всем маршрутам и переживают навигацию. **OS-level deep-linking отключён** (`flutter_deeplinking_enabled=false` в AndroidManifest, `FlutterDeepLinkingEnabled=false` в Info.plist): при переходе на Router API редирект OAuth `com.auskraft.kiseki://oauth` иначе попал бы и в go_router (спорная навигация / экран ошибки поверх рабочего OAuth). Редирект ловит `app_links` своим нативным каналом независимо (ADR-19); приложению OS-deep-link в карточки не нужен (личное офлайн-приложение). Модальные `showDialog`/`showModalBottomSheet` по-прежнему закрываются через `Navigator.pop(context, result)` — это оверлеи, а не маршруты go_router. `.route()`-фабрики на страницах удалены — путь к экрану теперь только через `AppRoute`.
...
CREATE TABLE media_items (
  ...
  media_type      TEXT    NOT NULL,                      -- movie|series|drama|anime
...
- `schemaVersion = 1` на старте; инкремент на каждое изменение схемы.
...
| `drift`, `drift_flutter`, `sqlite3_flutter_libs` | БД, открытие во Flutter, нативный SQLite (с FTS5) |
...
> **Факт (отклонения от плана):** `bloc_concurrency` НЕ используется — пере-подписка на стрим сделана вручную (`MediaListCubit._resubscribe`). `go_router` добавлен (A4 сделан): `MaterialApp.router` + `lib/app/router/app_router.dart`, плоские маршруты, навигация через `context.push`/`context.pop` (ADR-20). `drift_flutter` в зависимостях остался, но БД открывается через `NativeDatabase` напрямую (ADR-19) — можно убрать. Бэкап-пакеты добавлены и проверены на устройстве.
...
`media_type` (обязательно): `movie` · `series` · `drama` · `anime` — классификатор (иконки, фильтр, страна по умолчанию).
`format` (обязательно, ADR-07): `single` (нет эпизодов — фильм, аниме-фильм, OVA) · `episodic` (есть серии). Дефолт от типа, переопределяемо.
...
**Приблизительные даты.** У каждой из трёх пользовательских дат есть точность (`*_prec`: `day`/`month`/`year`). Пользователь может задать ориентировочную дату — UI показывает «12.03.2024» / «март 2024» / «2024» соответственно. Сортировка идёт по нормализованной дате (для `year` — 1 января, для `month` — 1-е число), так что приблизительные даты не теряют сортируемости. В пикере — переключатель гранулярности «День / Месяц / Год» + шорткаты «Сегодня», «Не помню».

### 6.7. Экраны

- **Главный (список/грид).** Переключатель грид↔список. Сверху горизонтальные полки: «Жду новые серии», «Смотрю сейчас» (+ опц. «Избранное»). FAB «+». Корзина — отдельный экран. В гриде — обложка-thumbnail, `title`, бейдж оценки, иконка статуса, мини-прогресс серий, флаг избранного.
...
- **Форма создания/редактирования** — прогрессивное раскрытие: обязательный минимум (`media_type`+авто-`format`, `title`, `status`) → основное (оригинал, год, страна, картинка, теги, оценка) → прогресс (только `episodic`) → статус-зависимое (`unfinished_reason`) → даты → заметка. Поля серий/причины показываются реактивно от `format`/`status`.
